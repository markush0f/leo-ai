...
from leo_realtime.leo_http import LeoHttpClient, LeoHttpError, resolve_conversation_id


def _client(handler) -> LeoHttpClient:
    transport = httpx.MockTransport(handler)
    return LeoHttpClient(
        "http://leo.test",
        timeout=5,
...
async def test_creates_conversation_then_chats() -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if request.url.path.endswith("/messages"):
            ...
            return httpx.Response(200, json={"text": f"leo:{body['text']}"})
        return httpx.Response(404)

    client = _client(handler)
    assert await client.chat("hola") == "leo:hola"
...
async def test_surfaces_server_error() -> None:
    ...
    with pytest.raises(LeoHttpError, match="mensaje vacío"):
        await client.chat("   ")
    await client.aclose()
