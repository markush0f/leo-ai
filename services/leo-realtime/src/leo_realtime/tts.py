...
def pocket_language(value: str) -> str:
    ...
    text = value.strip()
    if not text:
        raise ValueError("LEO_REALTIME_TTS_LANGUAGE is empty")
    try:
...
def create_tts(settings: Settings) -> FrameProcessor | None:
    ...
    Without TTS, leo mode still sends the agent reply as a JSON WebSocket text
    frame.
    """
    ...
    try:
        ...
        raise RuntimeError(
            'Pocket TTS no está instalado. En leo-realtime: pip install -e ".[tts]" '
