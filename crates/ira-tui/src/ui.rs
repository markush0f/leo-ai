...
fn draw_header(frame: &mut Frame, area: Rect, app: &App, config: bool) {
    ...
    frame.render_widget(
        Paragraph::new(header_line("ira", &right, area.width as usize)),
        area,
    );
}
...
mod tests {
    ...
    fn header_fills_between_brand_and_meta() {
        let text = line_text(&header_line("ira", "grok · grok-4.6", 32));
        assert!(text.starts_with("ira "));
    ...
    fn empty_chat_has_header_rule_and_prompt() {
        let mut app = crate::app::App::new(ira_store::stub_snapshot("grok", "grok-4.6", "x"));
        let text = screen_text(&mut app, 48, 12);
        assert!(text.contains("ira"));
