...
//! # Workspace crates
//!
//! - [`ira_api`] — snapshot, catalog mutations, and chat (tool loop included).
//!   [`ira_api::App::boot`] loads dotenv, tools, and PostgreSQL.
...
#[command(
    name = "ira-server",
    about = "API HTTP de Ira: chat y catálogo para el navegador"
)]
struct Cli {
    /// Listen address (`host:port`). `IRA_HTTP_BIND` overrides this when set.
    #[arg(long, default_value = "127.0.0.1:8787")]
    bind: String,
    /// Vite `dist` directory served at `/`. `IRA_WEB_ROOT` overrides this when set.
    #[arg(long)]
...
async fn main() {
    ...
    let cli = Cli::parse();
    let bind = std::env::var("IRA_HTTP_BIND")
        .ok()
        .filter(|s| !s.trim().is_empty())
        .unwrap_or(cli.bind);
    let web_root = std::env::var("IRA_WEB_ROOT")
        .ok()
        .filter(|s| !s.trim().is_empty())
        .map(PathBuf::from)
        .or(cli.web_root)
