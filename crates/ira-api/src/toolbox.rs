...
use ira_store::{self as db, DatabaseCipher, DatabaseConnectionRow};
use serde::Serialize;
use sqlx::PgPool;
...
pub async fn sync(pool: &PgPool, cipher: &DatabaseCipher) -> Result<(), String> {
    ...
    for row in active {
        ...
        let name = format!("ira-{}.yaml", row.id.simple());
    ...
    expected.push("ira-base.yaml".into());
...
pub async fn prepare() -> Result<PathBuf, String> {
    ...
    write_private(
        &dir.join("ira-base.yaml"),
        "kind: group\nname: ira_databases\ndescription: Bases de datos habilitadas en Ira.\ntools: []\n",
    )
    .await?;
    Ok(dir)
}
...
pub async fn reset() -> Result<(), String> {
    let dir = prepare().await?;
    remove_stale(&dir, &["ira-base.yaml".into()]).await
}
...
fn render(row: &DatabaseConnectionRow, password: &str) -> Result<String, String> {
    ...
    query_params.insert("application_name", "ira-ai");
...
async fn remove_stale(dir: &Path, expected: &[String]) -> Result<(), String> {
        ...
        .map_err(|err| format!("no se pudo leer configuración Toolbox: {err}"))?
    {
        ...
        if name.starts_with("ira-") && name.ends_with(".yaml") && !expected.contains(&name) {
