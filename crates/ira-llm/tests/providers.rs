use ira_llm::{ChatRequest, Client, ProviderId};
...
fn parses_ollama_tags() {
    let names =
        ira_llm::parse_ollama_tags(r#"{"models":[{"name":"gemma3:latest"},{"name":"gemma3:4b"}]}"#)
            .unwrap();
...
fn ollama_chat_uses_native_api() {
    ...
    let body = ira_llm::protocol::ollama_body("gemma3:latest", &req).unwrap();
...
fn parses_ollama_chat() {
    ...
    let resp = ira_llm::protocol::parse_ollama("gemma3:latest", raw).unwrap();
...
fn extracts_nested_http_error_message() {
    let msg = ira_llm::extract_error_message(
        r#"{"error":{"message":"model 'llama3.2' not found","type":"not_found_error"}}"#,
    )
    .unwrap();
    assert_eq!(msg, "model 'llama3.2' not found");
    let plain = ira_llm::extract_error_message(r#"{"error":"not found"}"#).unwrap();
...
fn openai_body_includes_messages() {
    let req = ChatRequest::user("hola").with_system("sé breve");
    let body = ira_llm::protocol::openai_body("grok-4.6", &req).unwrap();
...
fn claude_lifts_system_out_of_messages() {
    let req = ChatRequest::user("hola").with_system("sé breve");
    let body = ira_llm::protocol::claude_body("claude-sonnet-5", &req).unwrap();
...
fn parses_openai_choice() {
    ...
    let resp = ira_llm::protocol::parse_openai(ProviderId::Grok, "grok-4.6", raw).unwrap();
    assert_eq!(resp.text, "hey");
...
fn parses_claude_blocks() {
    ...
    let resp = ira_llm::protocol::parse_claude("claude-sonnet-5", raw).unwrap();
    assert_eq!(resp.text, "hola");
    assert_eq!(resp.provider, ProviderId::Claude);
}

fn sample_tool() -> ira_llm::ToolSpec {
    ira_llm::ToolSpec {
...
fn openai_wires_tool_result_messages() {
    let req = ChatRequest {
        messages: vec![
            ira_llm::ChatMessage::assistant_tools(
                "",
                vec![ira_llm::ToolCall {
                    id: "call_1".into(),
                    name: "appflowy_write".into(),
                    arguments: r#"{"title":"x"}"#.into(),
                }],
            ),
            ira_llm::ChatMessage::tool("call_1", "appflowy_write", r#"{"view_id":"abc"}"#),
        ],
        ..ChatRequest::default()
    };
    let body = ira_llm::protocol::openai_body("grok-4.6", &req).unwrap();
...
fn parses_ollama_tool_calls() {
    ...
    let resp = ira_llm::protocol::parse_ollama("llama3.2", raw).unwrap();
    assert_eq!(resp.tool_calls[0].name, "appflowy_write");
    assert!(resp.tool_calls[0].arguments.contains("Nota"));
}
