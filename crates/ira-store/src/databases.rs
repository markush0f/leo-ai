...
impl DatabaseCipher {
    /// Usa `IRA_MASTER_KEY` si existe. Si no, crea `.ira/master.key` y la reutiliza.
    pub fn from_env() -> Result<Self, DatabaseError> {
        if let Ok(raw) =
            std::env::var(MASTER_KEY_ENV).or_else(|_| std::env::var(LEGACY_MASTER_KEY_ENV))
        {
            let raw = raw.trim();
            if !raw.is_empty() {
...
fn master_key_path() -> PathBuf {
    ...
    for _ in 0..16 {
        if dir.join("Cargo.toml").is_file() && dir.join("crates").is_dir() {
            return dir.join(".ira/master.key");
        }
        if !dir.pop() {
            break;
        }
...
fn ensure_master_key(path: &Path) -> Result<(DatabaseCipher, String), DatabaseError> {
    migrate_legacy_master_key(path)?;
    if path.is_file() {
...
fn migrate_legacy_master_key(path: &Path) -> Result<(), DatabaseError> {
    if path.is_file()
        || path.parent().and_then(Path::file_name) != Some(std::ffi::OsStr::new(".ira"))
    {
        return Ok(());
    }
    ...
    let legacy = root.join(".leo/master.key");
    if !legacy.is_file() {
        return Ok(());
    }
    if let Some(parent) = path.parent() {
...
mod tests {
    ...
    fn creates_a_stable_master_key_without_env() {
        let root = std::env::temp_dir().join(format!("ira-master-{}", Uuid::new_v4()));
    ...
    #[test]
    fn migrates_the_legacy_master_key() {
        let root = std::env::temp_dir().join(format!("ira-master-migration-{}", Uuid::new_v4()));
        let legacy = root.join(".leo/master.key");
        let current = root.join(".ira/master.key");
        std::fs::create_dir_all(legacy.parent().unwrap()).unwrap();
        let encoded = base64::engine::general_purpose::STANDARD.encode([4_u8; 32]);
        std::fs::write(&legacy, format!("{encoded}\n")).unwrap();

        let (_, migrated) = ensure_master_key(&current).unwrap();
