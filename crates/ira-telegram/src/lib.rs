...
pub const HELP: &str = "\
Ira — escribe y te responde el modelo activo (el mismo catálogo que `ira`).
...
fn match_models<'a>(snap: &'a Snapshot, q: &str) -> Vec<&'a ira_store::ModelRow> {
...
fn match_providers<'a>(snap: &'a Snapshot, q: &str) -> Vec<&'a ira_store::ProviderRow> {
    let exact: Vec<_> = snap
        .providers
        .iter()
...
fn model_matches(snap: &Snapshot, model: &ira_store::ModelRow, q: &str) -> bool {
...
fn provider_matches(provider: &ira_store::ProviderRow, q: &str) -> bool {
    let q = q.to_ascii_lowercase();
...
mod tests {
    ...
    fn snap() -> Snapshot {
        ...
        s.models.push(ira_store::ModelRow {
            id: Uuid::from_u128(3),
            provider_id: pid,
            name: "grok-4.5".into(),
            effort: "low".into(),
    ...
    fn parse_strips_bot_username() {
        assert_eq!(parse_command("/model@IraBot grok"), Some(("model", "grok")));
