...
/// Veritas Kanban MCP client. Talks JSON-RPC to `/mcp`.
#[derive(Clone)]
pub struct Client {
    inner: Arc<Inner>,
}
...
/// One tool advertised by the Kanban MCP server (`tools/list`).
#[derive(Debug, Clone)]
...
impl Client {
    ...
    pub fn with_timeout(base_url: impl Into<String>, timeout: Duration) -> Self {
        ...
        Self {
            inner: Arc::new(Inner {
                http,
                mcp_url: mcp_endpoint(&base_url.into()),
    ...
    pub fn from_env() -> Option<Self> {
        env("VERITAS_MCP_URL").map(Self::new)
    }
...
pub(crate) fn mcp_endpoint(base: &str) -> String {
...
mod tests {
    ...
    fn mcp_endpoint_appends_path() {
        ...
        assert_eq!(
            mcp_endpoint("http://127.0.0.1:3100/mcp"),
            "http://127.0.0.1:3100/mcp"
        );
    }
