...
async fn lists_and_invokes_over_mcp() {
    ...
    let listed = leo_tools_db::list_tools::run(&client).await.unwrap();
    ...
    let exec = leo_tools_db::execute_sql::run(&client, "select 1")
        .await
        .unwrap();
    ...
    let tables = leo_tools_db::list_tables::run(&client, None).await.unwrap();
...
fn specs_use_db_prefix() {
    assert_eq!(leo_tools_db::list_tools::spec().name, "db_list_tools");
    assert_eq!(leo_tools_db::invoke::spec().name, "db_invoke");
    assert_eq!(leo_tools_db::execute_sql::spec().name, "db_execute_sql");
    assert_eq!(leo_tools_db::list_tables::spec().name, "db_list_tables");
    assert_eq!(leo_tools_db::list_schemas::spec().name, "db_list_schemas");
    assert_eq!(leo_tools_db::list_views::spec().name, "db_list_views");
    assert_eq!(
        leo_tools_db::get_query_plan::spec().name,
        "db_get_query_plan"
    );
    assert_eq!(
        leo_tools_db::database_overview::spec().name,
